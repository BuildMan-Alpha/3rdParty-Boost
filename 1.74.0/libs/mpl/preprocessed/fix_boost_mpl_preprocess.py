...
def check_header_comment(filename):
    ...
    sourcefile.close()
    # Search content for '$Id: fix_boost_mpl_preprocess.py,v 1.1 2020/09/08 21:22:10 kurt Exp $'.
    match = re.search(r'\$Id\$', content)
    if match == None:
        # Make sure that the correct value for '$Id: fix_boost_mpl_preprocess.py,v 1.1 2020/09/08 21:22:10 kurt Exp $' was already set.
        match = re.search(r'\$Id: fix_boost_mpl_preprocess.py,v 1.1 2020/09/08 21:22:10 kurt Exp $]+\$', content)
        if match != None:
...
def fix_header_comment(filename, timestamp):
    ...
    for line in fileinput.input( filename, inplace=1, mode="rU" ):
        # If header-comment already contains anything for '$Id: fix_boost_mpl_preprocess.py,v 1.1 2020/09/08 21:22:10 kurt Exp $', remove it.
        line = re.sub(r'\$Id: fix_boost_mpl_preprocess.py,v 1.1 2020/09/08 21:22:10 kurt Exp $]+\$', r'$Id: fix_boost_mpl_preprocess.py,v 1.1 2020/09/08 21:22:10 kurt Exp $', line.rstrip())
        # Replace '$Id: fix_boost_mpl_preprocess.py,v 1.1 2020/09/08 21:22:10 kurt Exp $' by a string containing the file's name (and a timestamp)!
        line = re.sub(re.escape(r'$Id: fix_boost_mpl_preprocess.py,v 1.1 2020/09/08 21:22:10 kurt Exp $'), r'$Id: fix_boost_mpl_preprocess.py,v 1.1 2020/09/08 21:22:10 kurt Exp $', line.rstrip())
        print(line)
