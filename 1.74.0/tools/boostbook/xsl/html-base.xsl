...
set       toc,title
  ...
  <xsl:template name="format.svn.revision">
    ...
    <!-- Remove the "$Date: 2020/09/08 21:40:54 $Date:: " -->
    <xsl:variable name="text.noprefix"
      ...
      <tr>
        <td align="left">
          ...
          <xsl:if test="count($revision-nodes) &gt; 0">
            ...
            <xsl:if test="string-length($revision-text) &gt; 0 and not($revision-text = '$Date: 2020/09/08 21:40:54 $')">
              <p>
                <small>
                  <xsl:text>Last revised: </xsl:text>
                  <xsl:choose>
                    <xsl:when test="not(contains($revision-text, '$Date: 2020/09/08 21:40:54 $revision-text, '$Date:: '))">
